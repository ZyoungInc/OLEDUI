...
public:
  ...
  uint16_t getStrWidth(const char *str) { return u8g2_GetUTF8Width(&u8g2, str); }
  uint16_t getUTF8Width(const char *str) { return u8g2_GetUTF8Width(&u8g2, str); }
  void drawStr(int16_t x, int16_t y, const char *str) { u8g2_DrawUTF8(&u8g2, x, y, str); }
  void drawUTF8(int16_t x, int16_t y, const char *str) { u8g2_DrawUTF8(&u8g2, x, y, str); }
  ...
  {
    u8g2_DrawUTF8(&u8g2, cursor_x, cursor_y, s);
    cursor_x += u8g2_GetUTF8Width(&u8g2, s);
  }
...
//磁贴变量
//所有磁贴页面都使用同一套参数
#define   TILE_B_FONT         u8g2_font_wqy14_t_gb2312    //磁贴大标题字体
...
//列表和文字背景框相关
#define   VOLT_FONT           u8g2_font_wqy14_t_gb2312    //电压数字字体
...
//文字编辑变量
static constexpr uint8_t TEXT_NAME_SLOTS = 4;
static constexpr uint8_t TEXT_CHAR_BYTES = 3;
static constexpr uint8_t TEXT_NAME_LEN = TEXT_NAME_SLOTS * TEXT_CHAR_BYTES;
static char   text_name[TEXT_NAME_LEN + 1] = "中文测试";
static const char text_choices[][TEXT_CHAR_BYTES + 1] = {
  "中",
  "文",
  "测",
  "试",
...
static constexpr uint8_t TEXT_CHOICES = sizeof(text_choices) / sizeof(text_choices[0]);
static uint8_t text_choice_index[TEXT_NAME_SLOTS] = { 0, 1, 2, 3 };
...
void ui_param_init();
static void text_edit_sync_buffer();
...
void layer_init_in()
{
  ...
  switch (ui.index)
  {
    ...
    case M_CHART:
      ...
      break;
    case M_TEXT_EDIT:
      text_choice_index[0] = 0;
      text_choice_index[1] = 1;
      text_choice_index[2] = 2;
      text_choice_index[3] = 3;
      text_edit_sync_buffer();
      text_edit_index = 0;
...
void tile_show(struct MENU arr_1[], const MAIN_ICON icon_pic[])
{
  ...
  const int16_t title_clip_y0 = TILE_TITLE_Y - u8g2.getAscent() - 3;
...
{ 
  ...
  {
    case 0: u8g2.print("禁用"); break;
    case 1: u8g2.print("音量"); break;
    case 2: u8g2.print("亮度"); break;
  }
}
...
{ 
  if (check_box.v[n - 1] == 0) u8g2.print("禁用");
...
void window_show()
{
  ...
  //需要在窗口修改参数时立即见效的函数
  if (win.select == DISP_BRI) u8g2.setContrast(ui.param[DISP_BRI]);
...
static void text_edit_sync_buffer()
{
  for (uint8_t slot = 0; slot < TEXT_NAME_SLOTS; ++slot)
  {
    memcpy(&text_name[slot * TEXT_CHAR_BYTES], text_choices[text_choice_index[slot]], TEXT_CHAR_BYTES);
  }
  text_name[TEXT_NAME_LEN] = '\0';
}

static void text_edit_step(bool forward, uint8_t index)
{
  if (index >= TEXT_NAME_SLOTS) return;
  if (forward)
  {
    text_choice_index[index] = (text_choice_index[index] + 1) % TEXT_CHOICES;
  }
  else
  {
    text_choice_index[index] = (text_choice_index[index] == 0) ? (TEXT_CHOICES - 1) : (text_choice_index[index] - 1);
  }
  text_edit_sync_buffer();
}
...
static void text_edit_ui_show()
{
  ...
  u8g2.drawStr((DISP_W - u8g2.getStrWidth("--文本编辑--")) / 2, 20, "--文本编辑--");
  u8g2.drawStr(10, 38, text_name);
  u8g2.drawStr(80, 50, "-返回");

  uint8_t box_x = 9;

  if (text_edit_index < TEXT_NAME_SLOTS)
  {
    if (text_blink < TEXT_BLINK_SPEED / 2)
    {
      for (uint8_t i = 0; i < text_edit_index; ++i)
      {
        char temp[TEXT_CHAR_BYTES + 1] = { 0 };
        memcpy(temp, &text_name[i * TEXT_CHAR_BYTES], TEXT_CHAR_BYTES);
        box_x += u8g2.getStrWidth(temp);
        box_x++;
      }
      char temp[TEXT_CHAR_BYTES + 1] = { 0 };
      memcpy(temp, &text_name[text_edit_index * TEXT_CHAR_BYTES], TEXT_CHAR_BYTES);
      u8g2.setDrawColor(2);
  ...
  {
    u8g2.setDrawColor(2);
    u8g2.drawRBox(78, 38, u8g2.getStrWidth("-返回") + 4, 16, 1);
    u8g2.setDrawColor(1);
  }
...
void text_edit_proc()
{
  ...
  switch (btn.id)
  {
    case BTN_ID_CC:
      ...
      {
        if (text_edit_index == 0) text_edit_index = TEXT_NAME_SLOTS;
    ...
    case BTN_ID_CW:
      ...
      {
        if (text_edit_index == TEXT_NAME_SLOTS) text_edit_index = 0;
        else text_edit_index += 1;
      }
      break;
    case BTN_ID_SP:
  ...
  switch (btn.id)
  {
    ...
    case BTN_ID_SP:
      switch (ui.select[ui.layer])
      {
        ...
        case 1:   window_value_init("亮度", DISP_BRI, &ui.param[DISP_BRI],  255,  0,  5, setting_menu, M_SETTING);  break;
        case 2:   window_value_init("磁贴动画", TILE_ANI, &ui.param[TILE_ANI],  100, 10,  1, setting_menu, M_SETTING);  break;
        case 3:   window_value_init("列表动画", LIST_ANI, &ui.param[LIST_ANI],  100, 10,  1, setting_menu, M_SETTING);  break;
        case 4:   window_value_init("窗口动画",  WIN_ANI,  &ui.param[WIN_ANI],   100, 10,  1, setting_menu, M_SETTING);  break;
        case 5:   check_box_m_select( SPOT_ANI );  break;
        case 6:   window_value_init("标签动画",  TAG_ANI,  &ui.param[TAG_ANI],   100, 10,  1, setting_menu, M_SETTING);  break;
        case 7:   window_value_init("淡出动画", FADE_ANI, &ui.param[FADE_ANI],  255,  0,  1, setting_menu, M_SETTING);  break;
